...
blocks will be executed:
  ...
  - again, once per-child process if the module is pulled in off of
    disk again via Apache::StatINC
  - again, only once in the parent process if PerlFreshRestart is On
...
executed: 
  ...
  - once per-child process if not pulled in by the parent process
  - again, once per-child process if the script file has changed on disk
  - again, only once if pulled in by the parent process via
    B<Apache::RegistryLoader> and PerlFreshRestart is On
