...
Mike Stok <mike@stok.co.uk> maintains mirrors of the mod_perl FAQ at:
...
A B<PerlScript> file is commonly used for intialization during server
startup time.  A PerlScript file name can be absolute or relative to
B<ServerRoot> or a path in C<@INC>.   The B<PerlScript> must return a
true value, i.e., the end of this file should have a: 
...
not reloaded.  To reload B<PerlScript>, B<PerlModule>'s, other
...
Normally, B<END> blocks are executed by Perl during it's C<perl_run()>
...
C<perl_run()> once, during server startup.  Any B<END> blocks
encountered during main server startup, i.e. those pulled in by the
B<PerlScript> or by any B<PerlModule> are suspended and run at server
shutdown, aka C<child_exit> (requires apache 1.3b1+).  Any B<END>
...
All other B<END> blocks encountered during other Perl*Handler callbacks,
...
alternative to B<END> blocks if this behavior is not desirable. 
...
=head2 Additional Memory Tips

=over 4

=item Server Startup

Use the B<PerlScript> and B<PerlModule> directives to load commonly
...
See Vivek Khera's L<mod_perl_tuning> document for more tips on
...
Currently for <Perl> sections to work, the C<PerlScript> configuration
directive must be defined, C</dev/null> will do just fine.
...
to listserv@listproc.itribe.net with the string "subscribe modperl"
in the body.  
