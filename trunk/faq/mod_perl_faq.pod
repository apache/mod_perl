...
needed to be loaded when the server starts up.  Put a PerlScript
...
subroutine that's pulled in with PerlScript or PerlModule, like so:
...
modperl> to C<majordomo@listproc.itribe.net>.
